...
This is a tricky one. Async functions taking mutable references are obviously very useful but, if we don't very carefully manage our calling pattern, crashes are almost inevitable. If you are using a framework (e.g. React) that 'calls you', it seems near-impossible to avoid the bad cases.
...
I'm interested to hear about any other pitfalls you've encountered while using wasm-bindgen. Do you have ideas for avoiding the situations outlined above? Let me know below! I'd like to add more scenarios to the [examples repo](https://github.com/rossng/rossng-wasm-pitfalls).
