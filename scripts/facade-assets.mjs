...
// printed at the end are what Facade.astro and globals.css expect.
...
// mirrored, rippled, smeared and tinted with the painting's water colour.
{
  ...
  const h = 140; // CSS px, must match --canal
...
// happens to put its bricks at nearly the tile's scale. The main coping line
// is 237px below the crop's top at source scale.
{
  const top = 103,
    w = 840;
  await emit(
